...
void basic_start(int columns,Cell cells[][columns]);
...
int main(){
	...
	while(!WindowShouldClose()){
		...
		if (!is_paused){
			...
			}else if (IsKeyPressed(KEY_UP) && fps < 120){
				fps++;
			}else if(IsKeyPressed(KEY_DOWN) && fps > 30){
				fps--;
			}else if (IsKeyPressed(KEY_R)){
				generation = 0;
				randomize_grid(&grid, cells);
			}
